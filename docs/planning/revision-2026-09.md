...
- [x] **`medicine`/`symptom` service layer; routes thinned to handlers; one `_visible_query()` — done 2026-09-25 (D3).** New `GlobalCatalogService[ModelType]` base (`app/core/base_service.py`, alongside `BaseTenantService`) with `_visible_query()` (`is_global OR tenant_id == caller's tenant`, which naturally narrows to global-only for a platform admin's `tenant_id=None` since SQLAlchemy compiles `== None` to `IS NULL`), `get_visible_or_404`, and `get_own_or_404` (tenant-owned, non-global — for update/delete). `MedicineService`/`SymptomService` (new `service.py` per module) hold all business logic; `routes.py` in both modules is now dependency wiring only, same routes/`response_model`s/status codes/authorization rules as before — a behavior-preserving refactor, including the pre-existing gap where not even an admin can edit a *global* row through these endpoints. Caught and fixed two real, pre-existing bugs while adding the test coverage that didn't exist for either module before this: (1) `SymptomResponse`/`SymptomAliasResponse`/`MedicineSymptomMappingResponse.tenant_id` were typed as required `str`, not `str | None`, even though D1 made the column nullable for global rows — fetching any global symptom/alias/mapping 500'd on response serialization; (2) `create_alias` on both modules passed `**data.model_dump()` (which already includes `medicine_id`/`symptom_id`, since `MedicineAliasCreate`/`SymptomAliasCreate` declare that field) alongside an explicit `medicine_id=`/`symptom_id=` kwarg — a duplicate-keyword `TypeError` on every alias-creation call. New `tests/integration/test_medicine_routes.py` (16 tests) and `test_symptom_routes.py` (15 tests) cover CRUD, admin-only global creation, tenant-isolation visibility (own/global/other-tenant), search, alias CRUD, and (medicine) mapping CRUD + cross-lookups. `pytest -q`: 473 passed / 1 xfailed / 0 failed (was 442 — the 31 new tests, zero regressions).
- [x] **RLS on the eight clinical tables — done 2026-09-26 (D2).** Migration `273747e56a3f` (`down_revision` = `ed07cf4aebc7`) adds `ENABLE`/`FORCE ROW LEVEL SECURITY` plus a `tenant_isolation` policy (`USING (tenant_id = current_setting('app.tenant_id', true))`, no explicit `WITH CHECK` — Postgres reuses `USING` for the check when none is given, which is what rejects a cross-tenant write) to `patients`, `appointments`, `visits`, `prescriptions`, `prescription_items`, `payments`, `invoices`, `patient_diagnoses` — all plain `TenantScopedModel` subclasses, one uniform policy shape. `get_current_user()` (`app/core/dependencies.py`) now sets the GUC per request via `set_config('app.tenant_id', tenant_id, true)` (transaction-scoped, can't leak across pooled-connection reuse between requests) in place of the dead `tenant_id_ctx` `ContextVar` it used to set-and-never-read — removed rather than left alongside the real mechanism. A platform admin (`tenant_id=None`) simply never sets it, which the policy treats as "no tenant" — zero rows from these 8 tables, a safe default since no current admin code path reads them. **Verified, but with a real caveat that has to stay attached to this entry, not get lost in a later summary: this is currently inert in both local dev and CI.** PostgreSQL superusers unconditionally bypass RLS — `FORCE ROW LEVEL SECURITY` only closes the table-*owner* loophole — and I confirmed directly that the local dev role (`imran`) has `rolsuper=True`; CI's bootstrapped `POSTGRES_USER` is a superuser too (standard Docker Postgres image behavior). There's a single `DATABASE_URL` everywhere in this codebase, no dedicated least-privileged application role anywhere. New `tests/integration/test_rls_policies.py` (3 tests) proves the policy logic is genuinely correct — not a no-op, not a tautology — by creating a throwaway non-superuser role and `SET ROLE`-ing into it (Postgres's RLS-bypass check is based on the *current* effective role, which `SET ROLE` does change even from a superuser login), then confirming an unscoped `SELECT` across two tenants' `patients`/`prescriptions` rows returns only the session's tenant, and a cross-tenant `INSERT` raises. I manually re-ran the same unscoped `SELECT` *without* `SET ROLE` first, as a control, and confirmed it leaks both tenants' rows — so the passing tests aren't a false positive of "RLS was never actually exercised." **Follow-up, named not silently assumed: introducing a dedicated non-superuser application role (and pointing `DATABASE_URL` at it) is what turns this from "correct but inert" into real production protection** — that's deployment/provisioning work, out of scope here. Migration reversibility checked (`upgrade` → `downgrade -1` → `upgrade` again, mirroring the CI `migration-safety` job). `pytest -q`: 476 passed / 1 xfailed / 0 failed (was 473 — the 3 new tests, zero regressions).
...
**Gate:** ~~an admin creates a global medicine successfully — the operation that returns a 500 today~~ **met 2026-09-24** (see D1 above); a doctor's prescription autocomplete returns seeded global medicines — not yet, no seed data exists; and a deliberately mis-written service query against a clinical table raises instead of returning another tenant's rows — **the policy logic is verified correct (D2, above) but the app doesn't yet connect as a role the policies actually constrain**, so this gate condition is proven-but-not-live, the most honest state short of the deployment work that would make it live. Stage 2 is not gated-complete: one of three conditions holds outright, one is proven-but-not-live.
