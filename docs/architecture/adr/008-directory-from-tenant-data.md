...
The public Practitioner Directory and Clinic Directory (scoped, not scheduled — see `docs/planning/future-scope-2026-09.md`, Track D) read from the existing `Tenant` and `User(role=doctor)` tables — filtered to `Tenant.is_approved = true` and a new opt-in `Tenant.is_publicly_listed: bool` (default `false`) — rather than a new `PractitionerProfile`/`ClinicProfile`/`directory_listings` table a tenant fills out separately.

A clinic's name, address, specializations, and doctor roster already live on `Tenant`/`User`/`DoctorDegree`/`DoctorTraining`. The directory is a `/api/v1/public/*` read path (`docs/planning/revision-2026-09.md` D4) over that same data, not a second copy of it.
...
2. **No second consumer today.** A dedicated profile table earns its cost when something needs to diverge from the operational record. Nothing does yet — the existing gate on this directory (§2.1, `revision-2026-09.md`) is "≥20 practitioners with complete public profiles," which the existing `Tenant`/`User` fields already satisfy.
...
- **This is the one call in the Track D batch with real reversal cost.** The "public projection" pattern gets baked into how the directory routes and services are written; unwinding it into a separate-table model later means a real migration, not a config flag. Flagged explicitly in `docs/planning/future-scope-2026-09.md` for that reason.
- **College/Institution Directory is unaffected** — colleges aren't tenants, so `colleges` is a genuine new `GlobalCatalogModel` table, admin-curated the same way Medicine/Symptom are (`docs/planning/future-scope-2026-09.md`). This ADR only concerns data that already has a tenant-owned source of truth.
